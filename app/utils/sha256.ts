...
export function hmacSha256Hex(key: string, value: string) {
  const blockSize = 64;
  ...
  const innerHash = sha256Bytes(concatBytes(inner, utf8Bytes(value)));
  const finalHash = sha256Bytes(concatBytes(outer, innerHash));
