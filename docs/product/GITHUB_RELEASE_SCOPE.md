...
- choose the repository visibility and license;
- reconcile the local history with the existing remote without rewriting either history by accident;
...
Current limitation: the remote contains an older, unrelated Git history. The cleaned client is newer, but publishing it requires an explicit history strategy.
